...
def make_vector(data, zero_test):
    ...
    if(is_long_and_sparse(vector, zero_test) == True):
        SparseVector(vector)
    else:
        FullVector(vector)
...
class Vector(object):
    ...
    def split(self):
        ...
        for i in range(0, len(self)/2):
            left_vector.append(self[i])
            count += 1
        for i in range(count, len(self)):
            right_vector.append(self[i])
...
class SparseVector(Vector):
    ...
    def __init__(self, values, indices, length = 0, \
        ...
        for i in self.data:
            if(i != 0):
                values.append(i)
                indices.append(self.data.index(i))
        length = len(values)
    ...
    def __setitem__(self, i, val):
        ...
        if i in self.data.indices:
            self.data.value[i] = val
        else:
            self.data.indices.append(i)
            self.data.value.append(val)
