...
def is_long_and_sparse(lst, zero_test = lambda x : (x == 0)):
    ...
    for element in lst:
        if zero_test(element):
            zeros += 1
    if(zeros != 0):
        density = float(zeros) / elements
        if elements > SIZE_THRESHOLD:
...
class Vector(object):
    ...
    def __eq__(self, vector):
        ...
        for idx, elem in self.components():
            if isinstance(vector, int) or (elem != vector[idx]):
                return False
        return True
...
class FullVector(Vector):
    ...
    def split(self):
        ...
        '''
        # Your Code
        return self.lst[:int(len(self)/2 + 0.5)],self.lst[:int(len(self)/2)]
...
class SparseVector(Vector):
    ...
    def merge(self, vector):
        ...
        # Your Code
        for i in range(len(vector)):
            if(not (self.length + i) in self.indices) and (vector[i] != 0):
                self.indices.append((self.length + i))
                self.values.append(vector[i])
