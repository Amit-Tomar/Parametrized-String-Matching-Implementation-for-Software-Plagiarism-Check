    ...
    if not is_long_and_sparse(data, zero_test):
        ...
        for i in range(len(data)):
            if not zero_test(data[i]):
                ...
                indices.append(i)
        vector_made = SparseVector(values, indices, len(data))
...
class Vector(object):
    ...
    def __add__(self, vector):
        ...
        add_vect_list = []
        for i in range(len(vector)):
            add_vect_list.append(self[i] + vector[i])
        sum_vector = make_vector(add_vect_list, self.zero_test)
        return sum_vector
    ...
    def __sub__(self, vector):
        ...
        sub_vect_list = []
        for i in range(len(vector)):
            sub_vect_list.append(self[i] - vector[i])
        sub_vector = make_vector(sub_vect_list, self.zero_test)
        return sub_vector
...
class FullVector(Vector):
    ...
    def merge(self, vector):
        ...
        for i in range(len(self)):
            merged_vector.append(self[i])
        for i in range(len(vector)):
            merged_vector.append(vector[i])
...
class SparseVector(Vector):
    ...
    def split(self):
        ...
        left = SparseVector(leftvalue, leftindex , self.length//2, self.zero_test)
        right = SparseVector(rightvalue, rightindex , self.length - self.length//2)
        return left, right
    ...
    def merge(self, vector):
        ...
        for i in range(len(self)):
            ...
            merge_index.append(self.indices[i])
        for i in range(len(vector)):
            merge_value.append(vector[i])
            merge_index.append(self.indices[i])
        merge_vect = SparseVector(merge_value, merge_index, self.length, vector.length)
        return merge_vect
