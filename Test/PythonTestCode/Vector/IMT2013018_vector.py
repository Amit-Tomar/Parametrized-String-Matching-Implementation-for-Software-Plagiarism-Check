...
def is_long_and_sparse(lst, zero_test):
    ...
    if len(lst) > SIZE_THRESHOLD:
        ...
        density_lst = count/len(lst)
        
        if density_lst >= DENSITY_THRESHOLD:
            return True
    return False 
    # Your Code
    ...
    if not is_long_and_sparse(data, zero_test):
        ...
        for i in range(len(data)):
            if not zero_test(data[i]):
                ...
                indices.append(i)
        vect = SparseVector(values, indices, len(data))
    return vect
    # Your Code
...
class Vector(object):
    ...
    def __add__(self, vector):
        ...
        sum_vect = []
        for i in range(len(vector)):
            sum_vect.append(self[i] + vector[i])
        vector = make_vector(sum_vect, self.zero_test)
        return vector
        # Your Code
    ...
    def __sub__(self, vector):
        ...
        vector = make_vector(sub_vector, self.zero_test)
        return vector
        # Your Code
...
class FullVector(Vector):
    ...
    def split(self):
        ...
        return make_vector(left_half, self.zero_test), make_vector(right_half, self.zero_test)
        # Your Code
...
class SparseVector(Vector):
    ...
    def split(self):
        ...
        for i in range(len(self)):
            if self.indices[i] < self.length//2:
                leftmat.append(self[i])
                leftindices.append(self.indices[i])
            else:
                rightmat.append(self[i])
                rightindices.append(self.indices[i])

        left_matrix = SparseVector(leftmat, leftindices , self.length//2, self.zero_test)
        right_matrix = SparseVector(rightmat, rightindices, self.length - self.length//2, self.zero_test)
    ...
    def merge(self, vector):
        ...
        for i in range(len(vector)):
            merge_mat.append(vector[i])
            merge_indices.append(self.indices[i])
        merge_vect = SparseVector(merge_mat, merge_indices, self.length, vector.length)
        return merge_vect
        # Your Code
