...
def is_long_and_sparse(lst, zero_test):
    ...
    len_list = len(lst)
    if count >= (DENSITY_THRESHOLD * len_list) and len_list >= SIZE_THRESHOLD:
        return True
    else:
        return False
    ...
    if is_long_and_sparse(data, zero_test) == True:
        ...
        for i in range (len(data)):
            if zero_test(data[i]) ==  False:
                ...
                index_list.append(i)
        vector = SparseVector(value_list, index_list, len(data), zero_test)
    else:
...
class Vector(object):
    ...
    def is_zero(self):
        ...
        for element in self.data:
            if self.zero_test(element) == False:
                return False
        return True
    ...
    def __add__(self, vector):
        ...
        for i in range(0, len(self)):
            sum_list.append(self[i] + vector[i]) 
        sum_vector = make_vector(sum_list, self.zero_test)
        return sum_vector
    ...
    def __sub__(self, vector):
        ...
        diff_vector = make_vector(diff_list, self.zero_test)
        return diff_vector
    ...
    def split(self):
        ...
        for i in range(0, len(self.data)):
            if i < (len(self.data)/2):
                left_list.append(self.data[i])
            else:
                right_list.append(self.data[i])
                
        left_vector = make_vector(left_list, self.zero_test)
        right_vector = make_vector(right_list, self.zero_test)
...
class SparseVector(Vector):
    ...
    def split(self):
        ...
        for i in range(0, (self.length/2)):
            if i in self.indices:
                index = self.indices.index(i)
                left_list.append(self.values[index])
            else:
                left_list.append(0)
        for i in range((self.length/2) + 1, self.length):
            if i in self.indices:
                index = self.indices.index(i)
                right_list.append(self.values[index])
            else:
                right_list.append(0)
        left_vector = make_vector(left_list, self.zero_test)
        right_vector = make_vector(right_list, self.zero_test)
    ...
    def merge(self, vector):
        ...
        # Your Code
        for i in range(len(vector)):
            self.values.append(vector.values[i])
            self.indices.append(vector.indices[i] + len(self))            
            

'''if __name__ == '__main__' :
    a4 = make_vector([2,2,2,2,3,3,3,3], lambda x: (x == 0))
    a5 = make_vector([4,4,4,4,5,5,5,5], lambda x: (x == 0))
    a6 = a5.merge(a4)
