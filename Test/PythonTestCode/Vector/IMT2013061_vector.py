...
def make_vector(data, zero_test):
    ...
    if is_long_and_sparse(data, zero_test):
        ind = []
        vec = []
        for a , variable in enumerate(data):
            if variable != 0:
                ind.append(a)
                vec.append(variable)
        len_of_vect = len(ind)       
        vector = SparseVector(vec ,ind ,len_of_vect ,zero_test)       
    else:
...
class Vector(object):
    ...
    def __add__(self, vector):
        ...
        if(len(self.data)==len(vector)):
            for a in range (len(self.data)):
                addition_of_vector = self.data[a]+vector[a]
                addition_of_vector.append(addition_of_vector)
            adding=make_vector(addition_of_vector,self.zero_test)
            return adding  
        else:
            return None
    ...
    def __sub__(self, vector):
        ...
        if(len(self) != len(vector)):
            ...
            vector_subtraction = []
            for a in range(len(self)):
                subtracting = self[a] - vector[a]
                vector_subtraction.append(subtracting)
                subtracting = make_vector(vector_subtraction, self.zero_test)
                return subtracting
    ...
    def __iadd__(self, vector):
        ...
        '''
        # Your Code
        min_vec = min(len(self), len(vector))
        for a in range(min_vec):
            self[a] += vector[a]
    ...
    def __isub__(self, vector):
        ...
        '''
        # Your Code
        min_vec= min(len(self.data),vector)
...
class FullVector(Vector):
    ...
    def split(self):
        ...
        if(len(self)%2 != 0):
            for a in range((len(self)/2)+1 , len(self)):
                sec_half.append(self[a])
            for b in range(0 , (len(self)/2)+1):
                fir_half.append(self[b])
        else:
            for a in range(len(self)/2 , len(self)):
                sec_half.append(self[a])
            for b in range(0 , len(self)/2):
                fir_half.append(self[b])
...
class SparseVector(Vector):
    ...
    def split(self):
        ...
        if(len(self)%2 == 0):
            ...
            a=len(self)/2+1
        for item in range(a):
            left.append(self[item])
            left_vector = make_vector(left,self.zero_test)
        for item in range(len(right)):
            right.append(self[item])
            right_vector = make_vector(right,self.zero_test)
