...
class Heap(object):
    ...
    def get_rightmostchild_index(self, parent_index):
        ...
        '''
        # Your code
        temp = (parent_index << self.exp2) + self.arity()
    ...
    def get_top_child(self, parent_index):
        ...
        if(childindex_end > self.size()):  
            temp = temp + self.data[childindex_start:]  
        else:
            temp = temp + self.data[childindex_start:childindex_end]
        if(childindex_start>=self.size()):
            return None
        if(self.min_top == False):
            val = max(temp)
            temp_index = temp.index(val)
            return temp_index + self.get_leftmostchild_index(parent_index)
        else:
            val = min(temp)
            temp_index = temp.index(val)
            return temp_index + self.get_leftmostchild_index(parent_index)
    ...
    def restore_subtree(self, i):
        ...
        while(i!=None and i<self.size()):
            ...
            if(self.min_top == False):
                if(self.cmp_function(element, self.data[i]) == 1):
    ...
    def remove(self, i):
        ...
        '''
        # Your code
        index_last_element = self.size()-1
        self.data[i], self.data[index_last_element] = self.data[index_last_element], self.data[i]
        k = self.data.pop()
        self.heapify()
        return k
