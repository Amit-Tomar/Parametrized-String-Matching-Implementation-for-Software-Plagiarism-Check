    ...
    if MIN_TOP:
        if leftchild!=None:
            for i in range(get_leftmostchild_index(parent_index),get_rightmostchild_index(parent_index)+1):
                if CMP_FUNCTION(DATA[leftchild] , DATA [i])==1:
                    leftchild = i
        return leftchild
    else:
        if leftchild!=None :
...
def restore_heap(i):
    ...
    if(MIN_TOP==True ):
        if(i != None ):
            ...
            if parentindex != None and CMP_FUNCTION( DATA[i] , DATA[parentindex])==1:
                while(i != None and parentindex != None  and CMP_FUNCTION( DATA[i] , DATA[parentindex]))==1:
        ...
        if(i != None ):
            parentindex = get_parent_index(i)
            if parentindex != None and CMP_FUNCTION(DATA[j]  , DATA[i])==-1:
                while(i != None and parentindex != None and CMP_FUNCTION(DATA[parentindex]  , DATA[i]))==-1:
                    DATA[i],DATA[parentindex]=DATA[parentindex],DATA[i]
                    i = parentindex
                    parentindex = get_parent_index(i)
            else:
