...
def restore_subtree(i):
    ...
    if (MIN_TOP==True):
        ...
        elif (CMP_FUNCTION(DATA[get_top_child(i)] , DATA[i]) == -1):
            c=get_top_child(i)
...
def restore_heap(i):
    ...
    '''if (MIN_TOP==True):
        if (get_parent_index(i)==None):
            return None
        elif (CMP_FUNCTION(DATA[i] , DATA[get_parent_index(i)] < 0)):
            k=get_parent_index(i)
            swap(i,get_parent_index(i))
            restore_heap(k)
        elif(CMP_FUNCTION(DATA[get_parent_index(i)] , DATA[i])<=0):
            restore_subtree(i)
    ...
    if (MIN_TOP==False):
        if (get_parent_index(i)==None):
            ...
            if (CMP_FUNCTION(DATA[i] , DATA[get_parent_index(i)] > 0)):
                k=get_parent_index(i)
                swap(i,get_parent_index(i))
                restore_heap(k)
            elif(CMP_FUNCTION(DATA[get_parent_index(i)] , DATA[i])>=0):
...
def heapify():
    ...
    if (last_parent_index == None):
        return None
    else:
        for i in range(0,last_parent_index+1):
            restore_subtree(last_parent_index - i )
