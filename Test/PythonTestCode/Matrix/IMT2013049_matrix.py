...
def remove_pad(matrix):
    remove_row = zero_append[0]
    remove_col = zero_append[3]
    print remove_row, remove_col
    correct_matrix = [[0]*remove_col]*remove_row
    for i in range(remove_row):
        for j in range(remove_col):
            correct_matrix[i][j] = matrix[i][j]
    ...
    if len(vector_list) > SIZE_THRESHOLD:
        ...
        if count/len(vector_list) > DENSITY_THRESHOLD:
            ...
            for index in range(len(vector_list)):
                if vector_list[index].is_zero() == False:
                    vect_list.append(vector_list[index])
...
class Matrix(object):
    ...
    def __add__(self, mat):
        ...
        for i in range(len(self)):
            sum_list.append(make_vector(self[i], zero_test = lambda x : (x == 0))+make_vector(mat[i],zero_test = lambda x : (x == 0) ))
        
        sum_matrix = make_matrix(sum_list)
    ...
    def __sub__(self, mat):
        ...
        for i in range(len(self)):
            sub_list.append(make_vector(self[i], zero_test = lambda x : (x == 0)) - make_vector(mat[i], zero_test = lambda x : (x == 0)))
        sub_matrix = make_matrix(sub_list)
        return sub_matrix
        # Your Code
    ...
    def merge_cols(self, mat):
        ...
        for i in range(len(self)):
            ...
            for val2 in mat[i]:
                lst.append(val2)
            merge_mat_list.append(lst)
        merged_matrix = make_matrix(merge_mat_list)
    ...
    def merge_rows(self, mat):
        ...
        for i in range(len(self)):
            merge_mat_list.append(self[i])
        for i in range(len(mat)):
            merge_mat_list.append(mat[i])
        merge_matrix = make_matrix(merge_mat_list)
        return merge_matrix
...
class SparseMatrix(Matrix):
        ...
        if type(key) == int:
            if key in self.indices:
                ...
                if key < len(self):
                    ...
                    matrix = make_vector(lst, zero_test = lambda x : (x == 0))
                    return matrix
        return None
    ...
    def merge_rows(self, mat):
        ...
        for row in range(len(self)):
            merge_vector.append(self[row])
            merge_indices.append(self.indices[row])
    
        for row in range(len(mat)):
            merge_vector.append(mat[row])
            merge_indices.append(len(self)+mat.indices[row])
...
#     vect1 = make_vector([1, 1, 1, 1,1,1,1], zero_test = lambda x : (x == 0))
#     vect2 = make_vector([1,1,1,1,1,1,1,1], zero_test = lambda x : (x == 0))
#     mat1 = make_matrix([vect1]*8)
#     mat2 = make_matrix([vect2]*7)
#      
#     mat3 = mat1*mat2
#     mat3 = remove_pad(mat3)
#     printmatrix(mat3)
    pass
