...
def remove_pad(matrix):
    ...
    n_list = [[0]*rem_col]*rem_row
    for i in range(rem_row):
        for j in range(rem_col):
...
def pad_matrix(vector_list):
    ...
    z_append.append(len(vector_list)) 
    z_append.append(len(vector_list[0])) 
    if nrow_pad == len(vector_list) and ncol_pad == len(vector_list[0]):
        return vector_list
    n_list = [[0]*ncol_pad]*len(vector_list)
    
    for i in range(len(vector_list)):
        for j in range(len(vector_list[0])):
            n_list[i][j] = vector_list[i][j]
    
    i = 0
    while( i < nrow_pad-len(vector_list)):
        n_list.append([0]*len(n_list[0]))
        i += 1
...
def make_matrix(vector_list):
    ...
    if len(vector_list) > SIZE_THRESHOLD:
        for vect in vector_list:
            if vect.is_zero():
                counter += 1
        if counter/len(vector_list) > DENSITY_THRESHOLD:
            vect_list = []
            indices = []
...
class Matrix(object):
    ...
    def left_right_split(self):
        ...
        left = []
        right = []
        for vect in range(len(self)):
            left.append(self[vect].split()[0])
            right.append(self[vect].split()[1])
        return left, right
    ...
    def __mul__(self, mat):
        ...
        if self.is_small() == True:
            ...
            p1 = top_left1 * (top_right2 - bottom_right2)
            p2 = (top_left1 + top_right1) * bottom_right2
            p3 = (bottom_left1 + bottom_right1) * top_left2
            p4 = bottom_right1 * (bottom_left2 - top_left2)
            p5 = (top_left1 + bottom_right1)*(top_left2 + bottom_right2)
            p6 = (top_right1 - bottom_right1)*(bottom_left2 + bottom_right2)
            p7 = (top_left1 - bottom_left1)*(top_left2 + top_right2)
...
class SparseMatrix(Matrix):
    ...
    def merge_rows(self, mat):
        ...
        for row in range(len(self)):
            ...
            merge_indice.append(self.indices[row])
    
        for row in range(len(mat)):
            merge_vector.append(mat[row])
            merge_indice.append(len(self)+mat.indices[row])
        merge_vect = SparseMatrix(merge_vector, merge_indice, self.length + mat.length )
...
'''if __name__ == '__main__':
    ...
    vect2 = make_vector([1,1,1], zero_test = lambda x : (x == 0))
    mat1 = make_matrix([vect1]*7)
