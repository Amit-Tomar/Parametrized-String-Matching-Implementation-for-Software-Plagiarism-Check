...
class Huffman(object):
    ...
    def compress(self, text_filename, arity_exp):
        ...
        code_stack, root_symbol  = self.build_huffman_tree(freq_table, 1)
        huff_hash, compress_len  = self.form_codes(root_symbol, code_stack)
        
        huff_filename = text_filename + ".txt" 
        self.write_compressed(text_filename, huff_filename, huff_hash, \
                              compress_len)
    ...
    def uncompress(self, huff_filename):
        ...
        chash, compress_len = self.read_codes(huff)
