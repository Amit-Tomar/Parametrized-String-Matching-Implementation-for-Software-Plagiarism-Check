...
class Huffman(object):
    ...
    def build_huffman_tree(self,freq_table, arity_exp):
        ...
        new_list = []
        huff_heap = Heap(True, arity_exp, self.cmp_freq)
        for i in freq_table:
            new_list.append((i, freq_table[i]))
        huff_heap.import_list(new_list)
        huff_heap.heapify()
        stack = []
    
        while(huff_heap.size()>1):
        
            element1 = huff_heap.pop()
            element2 = huff_heap.pop()
        
            
            huff_heap.add((element2[0]+element1[0], element1[1]+element2[1]))
            stack.append((element1[0], element1[1], element2[0]+element1[0], '1'))
            stack.append((element2[0], element2[1], element2[0]+element1[0], '0'))
        

        stack.append((huff_heap.DATA[0][0], huff_heap.DATA[0][1], "root", '1'))
        stack.append((huff_heap.DATA[1][0], huff_heap.DATA[1][1], "root", '0'))
    
        return stack,huff_heap.DATA[0][0]
    ...
    def form_codes(self,root_symbol, code_stack):
        ...
        print code_hash
        #while(i>=0):
        for elem in code_stack :
            for parent in code_stack:
                if parent[2] == elem[2]:
                    break
            code_hash[elem[0]] = parent[3] + elem[3]
            i -= 1
    ...
    def write_compressed(self,text_filename, huff_filename, codes, huff_length):
        ...
        for i in prim:
            ...
            if len(buff) == 8:
                ...
                buff = ''
            if len(buff) != 0:
                self.excessfunc(8 - len(buff))
                comp_str += chr(int(self.pad_to_nbits(buff, 1, 8-len(buff)), 2))
    ...
    def uncompress(self,huff_filename):
        ...
        code = self.read_codes(file_text_input)
        temp = ''
        bin_code = ''
        for bit in file_text_input.read():
            bin_code = bin(ord(bit))[2:]
            temp += self.pad_to_nbits(bin_code, -1, 8-len(bin_code))
        prim1 = ''
        seco1 = ''
        temp = temp[:(len(temp)-excess-8)]
