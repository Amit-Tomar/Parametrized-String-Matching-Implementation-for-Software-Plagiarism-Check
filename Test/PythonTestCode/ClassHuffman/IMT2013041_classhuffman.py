...
class Huffman(object):
    ...
    def build_huffman_tree(self, freq_table, arity_exp):
        ...
        for i in freq_table:
            heap.add((freq_table[i], i))
            
        while (len(heap.data)>1):
            elem1 = heap.pop()
            elem2 = heap.pop()
            composite_char = elem1[1] + elem2[1]
            composite_freq = elem1[0] + elem2[0]
            heap.add((composite_freq, composite_char))
            stack.append((elem1[1], elem1[0], composite_char, '0'))
            stack.append((elem2[1], elem2[0], composite_char, '1'))        
    ...
    def compress(self, text_filename, arity_exp):
        ...
        stack, root = self.build_huffman_tree(frequency_table, arity_exp)
    ...
    def uncompress(self, huff_filename):
        ...
        code_hash, length = self.read_codes(fil)
