...
class Huffman(object):
    ...
    def build_huffman_tree(self, freq_table, arity_exp):
        ...
        heap.import_list([(char, freq) for char, freq in freq_table.items()])
        heap.heapify()
    ...
    def write_compressed(self, text_filename, huff_filename, codes, huff_length):
        ...
        if(len(chunk) > 0):
            huff.write("{}".format(chr(int(self.pad_to_nbits(chunk, 1, 8), 2))))
    ...
    def uncompress(self, huff_filename):
        ...
        codes, huff_length = self.read_codes(huff)
        ...
        for i in range(0, len(compress_data)):
            bin_code += self.pad_to_nbits(bin(ord(compress_data[i]))[2:], -1, 8)
