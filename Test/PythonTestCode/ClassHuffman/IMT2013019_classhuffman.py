...
class Huffman(object):
    ...
    def build_huffman_tree(self,freq_table, arity_exp):
        ...
        stack = []
        for i in freq_table:
            min_heap += [(freq_table[i] , i)]
        min_heap = self.make_heap(min_heap)
        while(len(min_heap) > 1):
            min_heap[0] , min_heap[len(min_heap)-1] = min_heap[len(min_heap)-1] , min_heap[0]
            first_tuple = min_heap.pop()
            min_heap = self.make_heap(min_heap)
            min_heap[0] , min_heap[len(min_heap)-1] = min_heap[len(min_heap)-1] , min_heap[0]
            second_tuple = min_heap.pop()
            concat_tuple = (first_tuple[0] + second_tuple[0] , first_tuple[1] + second_tuple[1] , '')
            first_tuple = (first_tuple[1] , first_tuple[0] , concat_tuple[1] , '0')
            second_tuple = (second_tuple[1] , second_tuple[0] , concat_tuple[1] ,'1')
            min_heap += [concat_tuple]
            min_heap = self.make_heap(min_heap)
            stack += [first_tuple , second_tuple]
