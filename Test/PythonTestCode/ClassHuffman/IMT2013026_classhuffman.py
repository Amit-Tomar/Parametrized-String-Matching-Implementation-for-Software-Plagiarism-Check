...
class Huffman(object):
    ...
    def build_huffman_tree(self, freq_table, arity_exp):
        ...
        for elem in freq_table:
            ...
            self.modheap.add(elem)
    
        while(len(self.modheap.DATA)>2):
    
            element1 = self.modheap.pop()
    
            element2 = self.modheap.pop()
            
            stack.append((element1[0], element1[1], element2[0] + element1[0],1))
    
            stack.append((element2[0], element2[1], element2[0] + element1[0],0))
    
            self.modheap.add((element2[0] + element1[0], element1[1] + element2[1]))
    
        stack.append(( self.modheap.DATA[0][0], self.modheap.DATA[0][1],'root',1))
    
        stack.append(( self.modheap.DATA[1][0], self.modheap.DATA[1][1],'root',0))
    ...
    def write_compressed(self, text_filename, huff_filename, codes, huff_length):
        ...
        binary =  self.pad_to_nbits(binary, 1, NB)
