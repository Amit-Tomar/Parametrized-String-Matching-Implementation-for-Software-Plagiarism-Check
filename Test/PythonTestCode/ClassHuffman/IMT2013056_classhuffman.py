...
class Huffman(object):
    ...
    def build_huffman_tree(self,freq_table, arity_exp):
        ...
        '''
        # Your code
        self.modheap.initialize_heap(is_min=True, arity_exp=1, compare_fn=None)
        sample = [(y, x) for (x, y)in freq_table.items()]
        
        self.modheap.import_list(sample)
        self.modheap.heapify()
        
        sample = self.modheap.DATA
        
        stack = []
        while(self.modheap.size() >2):
            tup1 = self.modheap.DATA.pop(0)
            self.modheap.heapify()
            tup2 = self.modheap.DATA.pop(0)
            
            tup3 = (tup1[0] +tup2[0], tup1[1] +tup2[1])
            tup1 = (tup1[1], tup1[0], tup3[1], '0')
            tup2 = (tup2[1], tup2[0], tup3[1], '1')
            self.modheap.add(tup3)
            self.modheap.heapify()
    ...
    def compress(self, text_filename, arity_exp):
        ...
        '''
        # Your code
        fre_table = self.build_char_table(text_filename)
        codestack, root = self.build_huffman_tree(fre_table, arity_exp)
        codes, comp_len = self.form_codes(root, codestack)
