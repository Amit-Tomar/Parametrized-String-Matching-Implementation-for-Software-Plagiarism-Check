...
class Huffman(object):
    ...
    def build_huffman_tree(self, freq_table, arity_exp):
        ...
        form_stack.append((heap.data[0][0], heap.data[0][1], 'root', 1))
        form_stack.append((heap.data[1][0], heap.data[1][1], 'root', 0))
        return form_stack,''
    ...
    def write_compressed(self, text_filename, huff_filename, codes, huff_length):
        ...
        self.initialize(bit_size)
        binary = self.pad_to_nbits(binary, 1, bit_size)
        for i in binary:
            temp += i
            if len(temp) == 8:
    ...
    def compress(self, text_filename, arity_exp):
        ...
        self.write_compressed(text_filename, 'compressed.huff', codes, counter)
    ...
    def uncompress(self, huff_filename):
        ...
        codes, char = self.read_codes(compressed_file)
