...
class Huffman(object):
    ...
    def write_compressed(self, text_filename, huff_filename, codes, huff_length):
        ...
        for char in textfile.read():
            ...
            while (len(outstr) >= 8):
                ...
                outstr = outstr[8:]
        if (outstr != ''):
            outstr = self.pad_to_nbits(outstr, 1)
    ...
    def compress(self, text_filename, arity_exp):
        ...
        huff_stack, root_symb = self.build_huffman_tree(freq_table, arity_exp)
        codes, huff_length = self.form_codes(root_symb, huff_stack)
        huff_flname = text_filename.split('.')[0] + '.huff'
        self.write_compressed(text_filename, huff_flname, codes, huff_length)
        return huff_flname
    ...
    def uncompress(self, huff_filename):
        ...
        codes, huff_length = self.read_codes(huff_file)
        huff_code = ''
        for char in huff_file.read():
            compressed_char = bin(ord(char))[2:]
            compressed_char = self.pad_to_nbits(compressed_char, -1)
