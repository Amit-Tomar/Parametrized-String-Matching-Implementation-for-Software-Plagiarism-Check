...
class Huffman(object):
    ...
    def write_compressed(self, text_filename, huff_filename, codes, huff_length):
        ...
        huff.write(chr(int(self.pad_to_nbits(bitstr, 1, 8), 2)))
        huff.close()
    ...
    def compress(self, text_filename, arity_exp):
        ...
        codes, root = self.build_huffman_tree(freq_table, arity_exp)
        c_hash, length = self.form_codes(root, codes)
       
        self.write_compressed(text_filename, text_filename + '.huff', c_hash, length)
    ...
    def uncompress(self, huff_filename):
        ...
        ref, length = self.read_codes(readfile)
        ...
        for char in data:        
            ...
            if len(temp) < 8 :
                temp = self.pad_to_nbits(temp, -1, 8)                   
