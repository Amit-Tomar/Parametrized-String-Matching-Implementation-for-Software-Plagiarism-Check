...
class Huffman(object):
    ...
    def build_huffman_tree(self , freq_table, arity_exp):
        ...
        code_stack.append((heap.data[0][0], heap.data[0][1], "root", '0'))
        code_stack.append((heap.data[1][0], heap.data[1][1], "root", '1'))
        return code_stack , '' 
    ...
    def write_compressed(self , text_filename, huff_filename, codes, huff_length):
        ...
        if(len(variable)!=0):
            self.initialize( 8 - len(variable))
            compressed_form += chr(int(self.pad_to_nbits(variable, 1 , 8-len(variable)) , 2))
    ...
    def uncompress(self , huff_filename):
        ...
        code , length_compress = self.read_codes(files)
        variable = ''
        variable1 = ''
        variable2 = ''
        variable3 = ''
        for bits in files.read():
            ...
            variable += self.pad_to_nbits(variable1, -1, 8-len(variable1))
        variable = variable[:(len(variable)-self.NBITS-8)]
        for key in variable:
            variable2 += key
