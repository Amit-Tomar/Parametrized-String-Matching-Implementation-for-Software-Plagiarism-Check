...
def compress(text_filename, arity_exp):
    ...
    code_stack, root_symbol=build_huffman_tree(freq_table, arity_exp)
...
def uncompress(huff_filename):
    ...
    binary_letter_hash, compressed_length=read_codes(fp_read)
    fp_read=open(huff_filename, "r")
    ...
    for code in s:
        ...
        binary_str=pad_to_nbits(binary_str, -1, 8)
        bits+=binary_str
    temp=''
    bits=bits[:(SIZE_STR)]
