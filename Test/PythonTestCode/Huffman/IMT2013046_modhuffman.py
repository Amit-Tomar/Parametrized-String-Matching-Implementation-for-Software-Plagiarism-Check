...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    if(len(temporary)!=0):
        initialize(8 - len(temporary))
        compressed += chr(int(pad_to_nbits(temporary, 1 , 8-len(temporary)) , 2))
...
def uncompress(huff_filename):
    ...
    code , compressed_length = read_codes(huff_file)
    temporary = ''
    string = ''
    for bit in huff_file.read():
        string = bin(ord(bit))[2:]
        temporary += pad_to_nbits(string, -1, 8-len(string))
    temporary1 = ''
    temporary2 = ''
    temporary = temporary[:(len(temporary)-NBITS-8)]
