...
def form_codes(root_symbol, code_stack):
    ...
    comp_len = 0
    while(i >= 0):
        code[code_stack[i][0]] = code[code_stack[i][2]] + code_stack[i][3]
        if len(code_stack[i][0]) == 1:
            comp_len = comp_len + (len(code[code_stack[i][0]]) * code_stack[i][1])
        i = i - 1
    for key, value in code.items() :
        if len(key) != 1 :
            del code[key]
    return code , comp_len
