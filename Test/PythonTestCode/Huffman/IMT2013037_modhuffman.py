...
@author: raghavan
'''
from modheap import pop, size,DATA, initialize_heap, import_list, add,heapify, get_item_at
import modheap    
...
def build_huffman_tree(freq_table, arity_exp):
    ...
    initialize_heap(True, arity_exp , cmp_freq)
    import_list(freq_heap)
    heapify()
    min_heap = modheap.DATA
    stack = []
    heap_size = size()
    while(heap_size >= 2):
        tup1 = min_heap.pop(0)
        ...
        heapify()
        heap_size = size()
    root_element = min_heap[0]
    return stack, root_element
...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    for i in string:
        ...
        if(len(temp_string) == 8):
            ...
            temp_string = ''
    temp_string = pad_to_nbits(temp_string, 1, nbits = 8)
    comp_filename.write(chr(int(temp_string, 2)))
...
def compress(text_filename, arity_exp):
    ...
    # Your code
    extension = '.huff'
    char_dict = build_char_table(text_filename)
    stack, root = build_huffman_tree(char_dict, arity_exp)
    codes , compressed_len = form_codes(root, stack)
    write_compressed(text_filename, text_filename + extension, codes, compressed_len)
