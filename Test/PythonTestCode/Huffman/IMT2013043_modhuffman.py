...
def compress(text_filename, arity_exp):
    ...
    table = build_char_table(text_filename)
    
    stack, root = build_huffman_tree(table,arity_exp)
    
    code, compressed_length = form_codes(root,stack)
    
    write_compressed( text_filename , text_filename + '.huff' , code , compressed_length)
