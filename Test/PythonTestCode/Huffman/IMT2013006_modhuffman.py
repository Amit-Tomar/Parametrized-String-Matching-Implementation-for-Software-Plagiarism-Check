...
'''
import modheap
COMP_FUNCTION = lambda x, y : (1 if (x > y) else (-1 if (x < y) else 0))
...
def build_huffman_tree(freq_table, arity_exp):
    ...
    modheap.initialize_heap(True, arity_exp, COMP_FUNCTION)    
    modheap.import_list(freq_list)  
    freq_list=modheap.DATA  
    modheap.heapify()
    #2
    stack=[]
...
def compress(text_filename, arity_exp):
    ...
    stack,last_elem=build_huffman_tree(freq_table,arity_exp)
    code_stack,comp_len=form_codes(last_elem,stack)
    write_compressed(text_filename,text_filename+'.huff',code_stack,comp_len)
...
#             codes[line[0]]=line[1]
#         i+=1
    codes, compressed_length = read_codes(huff)
