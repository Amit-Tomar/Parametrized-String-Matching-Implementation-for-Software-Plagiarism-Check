...
def build_huffman_tree(freq_table, arity_exp):
    ...
    for ele in freq_table:
        modheap.add((ele,freq_table[ele]))
...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    for i in temp:
        ...
        if len(binary)==8:
            ...
            binary=''
    if len(binary)!=0:
        compressed += chr(int(pad_to_nbits(binary , 1 , 8-len(binary)) , 2))
...
def compress(text_filename, arity_exp):
    ...
    write_compressed(text_filename, text_filename+'.huff', code, count)
...
def uncompress(huff_filename):
    ...
    codes , length =read_codes(huff)
    t=''
    p=''
    for i in huff.read():
        p=bin(ord(i))[2:]
        t+=pad_to_nbits(p,-1,8-len(p))
    print  t
    t1=''
    t2=''
    t=t[:len(t)-NBITS-6]
    for i in t:
        ...
        if t1 in codes.keys():
            t2+=codes[t1]
            t1=''
    uncompress_file=open(huff_filename+'1','w')
