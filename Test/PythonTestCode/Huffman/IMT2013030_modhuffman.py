...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    conversion base.
    '''
    org_file, huff_file = open(text_filename,'r'), open(huff_filename, 'w')
...
def compress(text_filename, arity_exp):
    ...
    '''
    huffman_stack, root_element = build_huffman_tree(build_char_table \
                                                     (text_filename), arity_exp)
    codes, compressed_length = form_codes(root_element, huffman_stack)
    write_compressed(text_filename, "huff_file", codes, compressed_length)
    return "huff_file"
    # Your code
...
def uncompress(huff_filename):
    ...
    codes, compressed_length = read_codes(decode)
