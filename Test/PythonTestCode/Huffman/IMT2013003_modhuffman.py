...
def cmp_freq_small(char_tup1, char_tup2):
    '''
    Comparison function - compares two tuples (char, freq) on the frequency
    This is the function to be used to initialize the heap
    '''
...
def compress(text_filename, arity_exp):
    ...
    '''
    # Your code
    freq_char = build_char_table(text_filename)
    code_stack, root_symb = build_huffman_tree(freq_char, arity_exp)
    codes, huff_len = form_codes(root_symb, code_stack)
    huff_file = text_filename + '.huff'
    write_compressed(text_filename, huff_file, codes, huff_len)
    return huff_file
...
def read_codes(huff):
    ...
    # Your code
    codes_hash = {}
    line_huffman = huff.readline()
    codes_len, comp_len = line_huffman.split()
    codes_len = int(codes_len)
    comp_len = int(comp_len)
    while codes_len > 0:
        line_huffman = huff.readline().split()
        if len(line_huffman) == 1:
            codes_hash[line_huffman[0]] = ' '
        elif line_huffman[1] == '\\n':
            codes_hash[line_huffman[0]] = '\n'
        elif line_huffman[1] == '\\t':
            codes_hash[line_huffman[0]] = '\t'
        else :
            codes_hash[line_huffman[0]] = line_huffman[1]
        codes_len -= 1
    return codes_hash, comp_len
