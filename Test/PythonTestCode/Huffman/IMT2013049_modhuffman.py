...
def build_huffman_tree(freq_table, arity_exp):
    ...
    while len(modheap.DATA) > 2:
        ...
        modheap.add(composite_tup)
        stack.append((element1[0] , element1[1] , composite_tup[0] , '0'))
        stack.append((element2[0] , element2[1] , composite_tup[0] , '1'))
    stack.append((modheap.DATA[0][0], modheap.DATA[0][1], "root", '0'))
    stack.append((modheap.DATA[1][0], modheap.DATA[1][1], "root", '1'))
    return stack , ''                    
...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    if(len(temp)!=0):
        initialize(8 - len(temp))
        #NBITS = 8 - len(temp)
        compressed += chr(int(pad_to_nbits(temp, 1 , 8-len(temp)) , 2))
...
def compress(text_filename, arity_exp):
    ...
    code , huff_length = form_codes(root_element, code_stack)
    huff_file = text_filename + '.huff'
    write_compressed(text_filename , huff_file , code , huff_length)
    #look at it later
    return huff_file
...
def uncompress(huff_filename):
    ...
    code , compressed_length = read_codes(huff)
    temp = ''
    par = ''
