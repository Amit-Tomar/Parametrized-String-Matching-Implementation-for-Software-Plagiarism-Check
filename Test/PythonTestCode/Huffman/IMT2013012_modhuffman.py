...
def form_codes(root_symbol, code_stack):
    ...
    while(i>=0):
        tupl = code_stack[i]
        code_char[tupl[0]] = code_char[tupl[2]]+tupl[3]
            
        if(len(tupl[0])==1):
            compressed_length += len(code_char[tupl[0]])*tupl[1]
...
def compress(text_filename, arity_exp):
    ...
    stack,root_element = build_huffman_tree(freq, arity_exp)
    
    code_char,compressd_length = form_codes(root_element, stack)
    
    file_name = text_filename + ".txt"
    write_compressed(text_filename , file_name , code_char, compressd_length )  
...
def uncompress(huff_filename):
    ...
    uncompressed_file = huff_filename+'huff.txt'
    
    file_pointer = open(huff_filename, "r")
    codes = read_codes(file_pointer)
