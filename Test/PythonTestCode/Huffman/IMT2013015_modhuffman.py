...
def form_codes(root_symbol, code_stack):
    ...
    (ii) For every x of the code_stack (starting from the end) - form its code by appending the additional bit to 
...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    if len(chars[-1]) < 8:
        chars[-1] = pad_to_nbits(chars[-1], 1, 8)
...
def compress(text_filename, arity_exp):
    ...
    the_stack, sym = build_huffman_tree(freq, arity_exp)
    code_dict, huff_comp_len = form_codes(sym, the_stack)
    write_compressed(text_filename, text_filename + ".huff", code_dict, huff_comp_len)
