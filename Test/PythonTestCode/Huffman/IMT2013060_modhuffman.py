...
def write_compressed(text_filename, huff_filename, codes, huff_length):
    ...
    count = 0
    old_data = build_char_table(text_filename)
...
def compress(text_filename, arity_exp):
    ...
    stack = build_huffman_tree(freq_table, arity_exp)
    codes = form_codes(stack)
    write_compressed(text_filename, 'huff', codes, codes[1])
...
def read_codes(huff):
    ...
    for i in range(0,count):
        temp1 = huff.readline()
        temp1 = temp1.split()
        return_hash[temp1[0]] = temp1[1]
        
    return return_hash,int(temp[1])
...
def uncompress(huff_filename):
    ...
    hash,comp_length = read_codes(file1)
