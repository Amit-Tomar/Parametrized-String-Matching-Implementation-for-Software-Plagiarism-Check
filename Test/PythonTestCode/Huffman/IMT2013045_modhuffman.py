...
def compress(text_filename, arity_exp):
    '''
    Compress a given text file 'text_filename' using a heap with arity 2^arity_exp
    ...
    code_stack, top_root_element = build_huffman_tree( freq_table , arity_exp )
    codes, huff_length = form_codes( top_root_element , code_stack )
    write_compressed( text_filename, text_filename+".huff" , codes, huff_length )
...
def uncompress(huff_filename):
    ...
    hash_codes, compressed_length = read_codes(huff_ptr)
