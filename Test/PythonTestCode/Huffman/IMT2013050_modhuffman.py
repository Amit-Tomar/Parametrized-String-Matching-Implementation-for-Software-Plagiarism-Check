...
def form_codes(root_symbol, code_stack):
    ...
    global TOP_PARENT , TEST_FILE
    
    freq_table = build_char_table(TEST_FILE)
...
def compress(text_filename, arity_exp):
    ...
    code_stack , parent = build_huffman_tree(freq_table , arity_exp)
    
    hash_bin , compressed_len = form_codes('' , code_stack)
    huff_filename = str(text_filename) + ".huff"
    
    write_compressed(text_filename , huff_filename , hash_bin , compressed_len)
...
def uncompress(huff_filename):
    ...
    hash_of_codes , comp_len = read_codes(f_read)
