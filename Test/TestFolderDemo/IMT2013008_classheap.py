...
#     self.i=i
    ...
    # Your code
        ...
        for i in range((parent_index<<self.exp2)+1,((parent_index+1)<<self.exp2)+1):
            if self.min_top:
                if self.cmp_function(self.data[i],self.data[mins])==-1:
                    mins = i
            else:
                if self.cmp_function(self.data[i],self.data[mins])==1:
                    mins = i
    ...
    # Your code
        p=i
        while(self.get_rightmostchild_index(p) is not None):
            minx=self.get_leftmostchild_index(p)
            ...
            for j in range(self.get_leftmostchild_index(p),self.get_rightmostchild_index(p)+1):
                ...
                if self.cmp_function(self.data[j],self.data[maxx])==1:
                    maxx=j
            if self.min_top:
                if self.cmp_function(self.data[p],self.data[minx])==1:
                    self.data[p],self.data[minx]=self.data[minx],self.data[p]
                    minx=p
                else:
                    break 
            else:
                if self.cmp_function(self.data[p],self.data[maxx])==-1:
                    self.data[p],self.data[maxx]=self.data[maxx],self.data[p]
                    maxx=p
                else:
                    break
