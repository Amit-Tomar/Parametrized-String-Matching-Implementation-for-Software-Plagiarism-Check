...
Tree::Tree()
{
    ...
    rootNode = NULL;
    sourceFeomFileNumber = 1;
    rootNode = new Node();
}

void Tree::createSuffixTree(std::string sourceCode)
{

    Node * newNode = new Node(sourceCode.substr(sourceCode.length()-1),rootNode,sourceFeomFileNumber);
    rootNode->addChild(newNode);

    for( unsigned int i = 1 ; i < sourceCode.length() ; ++i )
    {
        std::string suffix = sourceCode.substr(sourceCode.length()-i-1);
        insertSuffix(rootNode,suffix,sourceFeomFileNumber);
    }

    ++ sourceFeomFileNumber;
}
